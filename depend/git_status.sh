...
for remote_name in $remotes; do
    ...
    if [ ! "$remote_latest_commit" = "$local_latest_commit" ]; then
        echo "[remote] commit: $remote_latest_commit $remote_name $remote_branch"
    fi
done
...
if (( branch_count > 1 )); then
    echo "[branch]: it has $branch_count branch"
fi
...
if [[ ! "$status_info" == *"$search_str"* ]]; then
  echo "[status] changes not staged for commit"
fi
