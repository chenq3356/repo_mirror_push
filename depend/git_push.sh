...
if [ $# -ne 4 ]; then
    echo "fatal: Usage: $0 <project_path> <remote_url> <remote_branch> <to_unshallow>"
    exit 1
fi
...
if [[ "$is_shallow" == "true" && "$to_unshallow" == "false" ]]; then
...
# 判断是否为浅克隆
if [[ "$is_shallow" == "true" && "$to_unshallow" == "true" ]]; then
